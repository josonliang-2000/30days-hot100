> 前置知识点
>
> 1. 一般默认限时1s ≈ 10^8语句频度， 即数据范围`n < 10^4`情况下，才可以使用`O(n^2)`的算法；
...
>
>    - 如何`O(1)`复杂度计算出以`height[i]`为顶部时可以获得的最大矩形：当高度为`height[i]`，只需求维持该高度的最大宽度，即分别获取`i`左边比height[i]小的最大下标` j`、`i`右边比`height[i]`大的最小下标`k`，最大宽度= k - j - 1。
>
>    - 对于`k`，相当于从`i`往右走，找到最近的比`height[i]`小的下标，故从左开始遍历，使用单调增栈，可求出所有的k；对于所有的`j`，则从右开始遍历即可。（为啥使用单调增栈？ —— 以求k为例，即遇到比当前大的进栈，遇到比当前小的出栈，出栈时收集k）
