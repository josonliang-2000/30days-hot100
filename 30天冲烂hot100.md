...
>
>    - 如何`O(1)`复杂度计算出以`height[i]`为顶部时可以获得的最大矩形：当高度为`height[i]`，只需求维持该高度的最大宽度，即分别获取`i`左边比height[i]小的最大下标` left[i]`、`i`右边比`height[i]`小的最小下标`right[i]`，最大宽度= `right[i] - left[i] - 1`。
